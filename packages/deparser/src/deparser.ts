...
import { getConstraintFromConstrType, objtypeName } from 'pgsql-enums';
...
export default class Deparser {
  ...
  InsertStmt(node: InsertStmt, context = {}) {
    const output = [];
    if (!node) {
      return new Error('no node! InsertStmt');
    }
    if (node.withClause) {
  ...
  DeleteStmt(node: DeleteStmt, context = {}) {
    const output = [''];
    if (!node) {
      return new Error('no node! DeleteStmt');
    }
    if (node.withClause) {
      output.push(this.WithClause(node.withClause, context));
    }
  ...
  UpdateStmt(node: UpdateStmt, context = {}) {
    const output = [];
    if (!node) {
      return new Error('no node! UpdateStmt');
    }
    if (node.withClause) {
      output.push(this.WithClause(node.withClause, context));
    }
  ...
  SelectStmt(node: SelectStmt, context = {}) {
    ...
    const e = new Error();
    console.log({node, context, stack: e.stack})
