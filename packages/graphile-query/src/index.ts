import {
  ...
  export const getSchema = async (pool: Pool, settings: GraphileSettings): Promise<GraphQLSchema> =>
  ...
  export class GraphileQuery {
    ...
    async query({ req = {}, query, variables, role }: QueryOptions) {
      const queryString = typeof query === 'string' ? query : print(query as any);
  ...
  export class GraphileQuerySimple {
    ...
    async query(query: string | object, variables?: Record<string, any>) {
      const queryString = typeof query === 'string' ? query : print(query as any);
      return await withPostGraphileContext(
        {
          pgPool: this.pool
